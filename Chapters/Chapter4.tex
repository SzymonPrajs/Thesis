...
\subsection{Theory}
Gaussian Process (GP) is defined as a set of normally distributed random variables, (x, y), where any subset of them can be drawn from a joint Multivariate Gaussian Distribution, defined using a mean function, $m = m(x)$, and covariance function, K = k(x, x'). The process of interpolating data using GPs is refered to as Gaussian Process Regression (GPR) or Krigging.
