...
In order to create a modern set of templates, suitable for ML studies of SNe, we propose a new set of tools which move away from the approach of using the master templates found in \citet{Nugent2006}. We use all availble, high quality photometric and spectroscopic data for CCSN found in the literature in order to create templates that are not correlated with each other. We also use auxiliry UV data to extend the SED to lower wavelengths allowing, for the first time, to simulate these objects at in all bands in their detectibility range in DES and LSST. 
