...
\subsection{Theory}
Gaussian Process (GP) is defined as a set of normally distributed random variables where any subset of them can be drawn from a joint Multivariate Gaussian Distribution, defined entirely using a mean function $m = m(x)$ and covariance function, K = k(x, x'). The process of interpolating data using GPs is refered to as Gaussian Process Regression (GPR) or Krigging.

\begin{equation}
\begin{pmatrix} y \\
y_* \end{pmatrix} \sim N\Biggl(m,\begin{pmatrix} K & K'_*\\
