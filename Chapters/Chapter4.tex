...
\subsection{Theory}
Gaussian Process (GP) is defined as a set of normally distributed random variables where any subset of them can be drawn from a joint Multivariate Gaussian Distribution, defined entirely using a mean function $\bar{f}(\mb{x})$ and covariance function $k(\mb{x},\mb{x}')$ \\. The process of interpolating data using GPs is refered to as Gaussian Process Regression (GPR) or Krigging.

\begin{equation}
\begin{pmatrix} y \\
y_* \end{pmatrix} \sim N\Biggl(0,\begin{pmatrix} K & K'_*\\
 K_* & K_{**} \end{pmatrix}\Biggr),
\end{equation}
where $K$ is the covariance matrix for the labeled points, $K_*$ is the covariance vector between the new point and the labeled points, and $K_{**}$ is the inherent measurement noise.
Then,
\begin{equation}
p(y_*|y) \sim N(K_*K^{-1}y,K_{**}-K_*K^{-1}K'_*).
\end{equation}

 also often refered to as Krigging, is a technique based on the principle that the observed data points are normally distribution, connected using a common kernel, K, and drown from a Multivariate Gaussian Distribution with a mean matching that of the data.
