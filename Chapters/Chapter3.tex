...
The rate of any transient event is one of its most fundamental properties which provides clues about the origin and the evolution of the events throughout the history of the universe. In this chapter I present the measurement of a volumetric rate of SLSNe at z$\sim$1 measured using the archival SNLS data. This follows the rate measurements for SNIa \citep{Perrett2012} and CCSN \citep{Bazin2009} providing a unique opportunity to compate all there measurements using a homogeneous dataset. For SLSNe, the rate measurement provides a unique opportunity to test their progenitor models. It has been knows, since they were first identified, that SLSNe are an extremely rare class of events with (in the local universe) only approximately one in 10000 CCSN becoming resulting in a SLSN event. Such low rate suggests that the origin of SLSNe must be connected with an very rare progenitor scenario. By quantifying their rareness we canexclude some models or alternatively quantify the regions of the parameter spaces for others teaching us valuable lessons aboout the origin of these uniquely energetic events.

Before this work, three previous measurements of the rate of SLSNe have been published. A local rate of SLSNe was calculated using a single SLSN-I events dicovered by the Texas Supernova Search (TSS) \citep{Quimby2014}. Their result shows the rate, $\rho_{\mathrm{SLSN}} = 19^{73}_{17}$ SNe Gpc$^{-3}$ Yr$^{-1}$ at the volume weighted redshift of z=0.23 with the vast uncertainty range can be attributed to the scarsity of their sample. \citet{Cookie2013} presented a rate of SLSNe at a volume weighted z$\sim$3 finding using two SLSNe discovered in the deep stacked SNLS data. They estimate $\rho_{\mathrm{SLSN}}$ = 200 SNe Gpc$^{-3}$ Yr$^{-1}$, however, do not provide an uncertainty estimate which we must assume, making a conservative estimate to be 50\%. The final of the three measurements was performed by \citet{McCrum2015} using a sample of spectroscopically confirmed SLSNe in the PanSTARRS survey. Their results hoewever used a different approach basing the rate of SLSNe on the ralative rate and spectroscopic completeness compared to that of CCSNe discovered in the same data set. While we include it for comparison, the values cannot be directrly compared.
