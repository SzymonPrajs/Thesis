...
\subsubsection{Data preparation and model selection}
...
\subsubsection{Training the model}
- The final model was used on the training sample and acihieved anaccuracy of 90\% anaccuracy
- The classification rate can be split individually for each objects and while this does not necessarily makes sense it shows that real SN\,Ia are very likely to be selected as Ia's in this model.
- This is basically the best classifier of SN despite not having any redshift information which is super fucking awesome and very encouraging.

\subsection{SLSNe in DES}
- I have applied the model to the spectroscopically idendified objects and find that almost all SN\,Ia are classes correctly and the same is true for CCSN.
- Amongst SLSN that is less true with only 12 of 17 objects entering the sample. The remaining 5 objects are identified as CCSNe
- This is not great but greatness was never expected, specially that the objects that were missidentified we hard.
