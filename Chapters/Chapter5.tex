...
\subsubsection{Choosing the training sample}
...
Due to their complexity, CNNs rely on heavily fine-tuned optimisers for minimising the model's loss functions. As a result, the data must sitisfy a number of strict criteria in order to comply with these constrains. While most of them are not relevant to the transient data, we must observe the limit on the upper and lower limits of the flux which should range between positive and negative unity.

As the final piece of book-keeping, the data, must be dimentially reshapes in order to match the directions in the parameters space in which we wish to identify the features. These are; time, colour, season. [NEED TO WRITE HOW TIME AND SEASON ARE NOT THE SAME THING]
...
This has undergone many iterations and levels of complexity. In all instances the first point of access in the model is a convolutional layer containing 30-80 independant filters each between 3-9 epochs wide. These filters are applied to each season and band independantly
